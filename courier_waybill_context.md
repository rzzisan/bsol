...
> **✅ RESOLVED (2026-08-12) — বাংলা টেক্সট এখন real OpenType shaping (HarfBuzz) দিয়ে রেন্ডার হয়, regex-reorder hack না।** §৪.৫-এ বহুদিন ধরে "OPEN/UNSOLVED" হিসেবে চিহ্নিত ছিল — root cause শেষমেশ পাওয়া গেছে: regex matra-reordering কখনোই conjunct consonant ligature (উদ্দিন, স্বাস্থ্য, মনিরুজ্জামান-এর মতো অতি-সাধারণ যুক্তাক্ষর) বানাতে পারত না, যেটাই আসল "ভাঙ্গা ভাঙ্গা" দেখানোর কারণ ছিল, matra-order ভুল থাকা না। সমাধান ও নতুন আর্কিটেকচার পুরোপুরি লেখা আছে **§৪.৭**-এ — নতুন কেউ বাংলা রেন্ডারিং নিয়ে কাজ করতে এলে ওটাই প্রথমে পড়ো। **স্কোপ নোট:** এখন পর্যন্ত শুধু waybill-এর `classic` টেমপ্লেট + order invoice-এ wire করা হয়েছে (ব্যবহারকারীর নির্দিষ্ট অভিযোগ অনুযায়ী) — বাকি ২১টা sticker টেমপ্লেট এখনো পুরনো regex-reorder পদ্ধতিতেই আছে, একই প্যাটার্নে extend করা যাবে (§৪.৭-এর "বাকি টেমপ্লেটে extend করতে" অংশ দেখো)। §৪.৫ (পুরনো investigation history) নিচে অপরিবর্তিত রাখা হয়েছে রেফারেন্সের জন্য, কিন্তু এখন সেটা "সমাধান হয়ে গেছে" হিসেবে বোঝা উচিত, §৪.৭ অনুযায়ী।
...
- **স্কোপ (ইচ্ছাকৃতভাবে সীমিত করা হয়েছে):** waybill-এর `classic` টেমপ্লেট (customerName/address/fromLine/shopAddress/itemsSummary/notes) এবং `OrderInvoicePdfService` (shopName/shopAddress/customerName/address/প্রতি item-এর name+sku·variant/notes) — ইউজারের নির্দিষ্ট অভিযোগ ("courier sticker and order invoice") অনুযায়ী। বাকি ২১টা sticker টেমপ্লেট (pathao + ২০টা নতুন) **এখনো পুরনো regex-reorder টেক্সট রেন্ডারিং ব্যবহার করছে** — কোনো রিগ্রেশন না (আগের মতোই আচরণ), কিন্তু matra-order ঠিক থাকলেও conjunct ligature এখনো ফর্ম হবে না ওগুলোতে।

**বাকি টেমপ্লেটে extend করতে (ভবিষ্যতে):** প্রতিটা টেমপ্লেটের জন্য (ক) সেই টেমপ্লেটের geometry-তে `contentWidthMm` (বা সমতুল্য wrap-width) আছে কিনা চেক/যোগ করা, (খ) `render()`-এর label-building-এ সেই টেমপ্লেটের Bengali ফিল্ডগুলোর জন্য `queueShapingJob()` কল যোগ করা (raw text দিয়ে, reordered না!), (গ) সংশ্লিষ্ট Blade partial-এ img/fallback প্যাটার্ন যোগ করা। `classic` টেমপ্লেটের এই সেশনের ডিফ-টাই রেফারেন্স ইমপ্লিমেন্টেশন।
