...
> এই ফাইলটা AI agent-দের জন্য: courier waybill/sticker PDF (এবং একই কোড-প্যাটার্ন শেয়ার করা order sales invoice, `OrderInvoicePdfService`) নিয়ে কোনো কাজ করার আগে পুরো কোডবেস স্ক্যান না করে এই ফাইল পড়লেই যথেষ্ট। শেষ আপডেট: 2026-08-17 (প্রথম ডেলিভারি + একাধিক dompdf বাগ ফিক্স + শপ প্রোফাইল ইন্টিগ্রেশন + অর্ডার ইনভয়েস ফিচার + COD amount ফিক্স + Pathao-স্টাইল লেবেল + Sticker Template ফিচার (২২টা টেমপ্লেট, প্রিভিউ থাম্বনেইল সহ, §৬) + **✅ বাংলা টেক্সট রেন্ডারিং বাগ ফাইনালি সমাধান — real HarfBuzz shaping, সবগুলো ২২টা টেমপ্লেট + order invoice-এ (§৪.৭)** + **order invoice-এ Payment History টেবিল যোগ (§৮.১)**, একাধিক দিনে একাধিক সেশনে)। এই ফিচার `feature_roadmap_context.md`-এর #5 আইটেম — সম্পূর্ণ ও deployed। **নতুন কোনো টেমপ্লেট/PDF লে-আউট বানানোর আগে §৬.৪ পড়ো** (dompdf-এর box-sizing না-থাকা নিয়ে fundamental বাগ-ক্লাস) **এবং বাংলা টেক্সট নিয়ে কাজ করলে §৪.৭ পড়ো** (real shaping আর্কিটেকচার, raw vs reordered text নিয়ে একটা ক্রিটিক্যাল গটচা আছে সেখানে)।
>
...
> **✅ RESOLVED (2026-08-12) — বাংলা টেক্সট এখন real OpenType shaping (HarfBuzz) দিয়ে রেন্ডার হয়, regex-reorder hack না।** §৪.৫-এ বহুদিন ধরে "OPEN/UNSOLVED" হিসেবে চিহ্নিত ছিল — root cause শেষমেশ পাওয়া গেছে: regex matra-reordering কখনোই conjunct consonant ligature (উদ্দিন, স্বাস্থ্য, মনিরুজ্জামান-এর মতো অতি-সাধারণ যুক্তাক্ষর) বানাতে পারত না, যেটাই আসল "ভাঙ্গা ভাঙ্গা" দেখানোর কারণ ছিল, matra-order ভুল থাকা না। সমাধান ও নতুন আর্কিটেকচার পুরোপুরি লেখা আছে **§৪.৭**-এ — নতুন কেউ বাংলা রেন্ডারিং নিয়ে কাজ করতে এলে ওটাই প্রথমে পড়ো। **স্কোপ নোট (2026-08-12 আপডেট):** শুরুতে শুধু waybill-এর `classic` টেমপ্লেট + order invoice-এ wire করা হয়েছিল; একই দিনেই বাকি ২১টা sticker টেমপ্লেটেও (pathao + ২০টা) একই real-shaping প্যাটার্ন extend করা হয়েছে — **এখন সবগুলো ২২টা টেমপ্লেট + order invoice real HarfBuzz shaping ব্যবহার করে, regex-reorder আর কোথাও প্রাইমারি পাথ না** (শুধু Node/শেপিং ব্যর্থ হলে fallback হিসেবে থেকে গেছে)। প্রতিটা টেমপ্লেট মনিরুজ্জামান+মিশ্র ঠিকানা দিয়ে `pdftoppm` রেন্ডার করে ভিজুয়ালি ভেরিফাই করা হয়েছে। §৪.৫ (পুরনো investigation history) নিচে অপরিবর্তিত রাখা হয়েছে রেফারেন্সের জন্য, কিন্তু এখন সেটা "সমাধান হয়ে গেছে" হিসেবে বোঝা উচিত, §৪.৭ অনুযায়ী।
...
### ৪.৫ Complex-script (বাংলা) শেপিং নেই — matra reordering নিজেই করতে হয় — ✅ **RESOLVED 2026-08-12, §৪.৭ দেখো** (নিচের investigation history রেফারেন্সের জন্য অপরিবর্তিত রাখা হয়েছে)
...
### ৪.৭ ✅ বাংলা টেক্সট real শেপিং দিয়ে ফিক্স (HarfBuzz + SVG) — 2026-08-12

**রুট কজ ফাইনালি পাওয়া গেছে:** ইউজার বারবার রিপোর্ট করছিল "বাংলা টেক্সট ভাঙ্গা ভাঙ্গা আসে" — §৪.৫-এর পুরো ইনভেস্টিগেশন matra-**order** নিয়ে ছিল (এবং সেটা মূলত ঠিকই ছিল, poppler টেস্টে confirmed)। আসল সমস্যা ছিল সম্পূর্ণ ভিন্ন জিনিস: **conjunct consonant ligature**। বাংলায় virama-জোড়া consonant cluster (উদ্দিন-এর দ্দ, স্বাস্থ্য-এর স্ব/স্থ্য, মনিরুজ্জামান-এর জ্জ — এক্সট্রিমলি কমন, প্রায় প্রতিটা নাম/ঠিকানায় থাকে) সঠিকভাবে দেখাতে ফন্টের OpenType **GSUB** ligature-substitution লাগে। dompdf-এর কোনো shaping engine নেই (§৪.৫-এ যা প্রমাণিত), তাই GSUB কখনোই apply হতো না — regex matra-reorder ভালো থাকলেও, conjunct-জোড়া প্রতিটা অক্ষর **আলাদা আলাদা**, মাঝে একটা visible hasant (্) মার্ক নিয়ে রেন্ডার হতো (`উদ্‌দিন` না, `উ` + `দ` + hasant-hook + `দ` + `ি` + `ন` — চোখে "ভাঙ্গা" লাগে, কারণ আসল বাংলা টাইপোগ্রাফিতে এগুলো সবসময় কম্প্যাক্ট ligature হিসেবে দেখায়)। ফন্ট ফাইলেই GSUB/GPOS টেবিল আছে কিনা raw TTF table directory পার্স করে ভেরিফাই করা হয়েছে (`NotoSansBengali-{Regular,Bold}.ttf` দুটোতেই GSUB+GPOS+GDEF আছে) — সমস্যা ফন্টে না, dompdf-এর রেন্ডারিং পাইপলাইনে।

**সমাধান — dompdf-কে বাইপাস করে real shaping:**
1. **`resources/bengali-shaper/`** — একটা ছোট standalone Node.js প্রজেক্ট (নিজস্ব `package.json`, backend-এর PHP কোডের বাইরে)। ডিপেন্ডেন্সি: শুধু **`harfbuzzjs`** (real HarfBuzz-এর WASM বিল্ড — Chrome/Android যেটা ব্যবহার করে, ঠিক সেটাই)। `npm install`-এ কোনো native compilation লাগে না (কোনো gcc/make/python-dev এই সার্ভারে নাই, তাও কাজ করে, কারণ pure-WASM প্যাকেজ)।
2. **`shape.mjs`** — stdin-এ JSON নেয় (`{jobs: [{id, text, fontPath, latinFontPath, fontSizePx, maxWidthPx, color}]}`), প্রতিটা job-এর জন্য:
   - Text-কে **script দিয়ে সেগমেন্ট করে** (Bengali run vs বাকি সব) — কারণ (ক) HarfBuzz-এর নিজের `guessSegmentProperties()` পুরো বাফারের জন্য একটা মাত্র script guess করে, মিক্সড টেক্সটে (বাংলা ঠিকানা + ইংরেজি thana/district নাম, যেটা এই অ্যাপে খুবই কমন) ভুল script guess করত এবং ইংরেজি অংশও বাংলা GSUB দিয়ে শেপ করার চেষ্টা করত (ফলাফল: `.notdef` বক্স-গ্লিফ); (খ) **`NotoSansBengali`-এর কোনো Latin গ্লিফই নাই** (cmap টেবিল চেক করে কনফার্ম করা হয়েছে — প্রতিটা ASCII লেটার `.notdef`/glyph 0-এ ম্যাপ হয়) — তাই Latin রান-এর জন্য **`DejaVuSans.ttf`** ব্যবহার করা হয় (dompdf নিজেই এই ফন্ট ব্যবহার করে, `vendor/dompdf/dompdf/lib/fonts/DejaVuSans{,-Bold}.ttf`-এ আগে থেকেই আছে)।
   - প্রতিটা রান আলাদা `hb.Buffer` দিয়ে শেপ করে (explicit `buffer.setScript()`), গ্লিফ আউটলাইন **`font.glyphToPath()`** দিয়ে সরাসরি SVG path হিসেবে বের করে (আলাদা rasterizer/`opentype.js` লাগে না, harfbuzzjs নিজেই দেয়)।
   - **Word-wrap নিজে ম্যানেজ করে** — greedy wrap, প্রতিটা candidate line-এর real shaped-width চেক করে (character-count estimate না, কারণ ligature-এর কারণে conjunct-heavy টেক্সট অপ্রত্যাশিতভাবে সরু হয়ে যায়)।
   - সব লাইন মিলিয়ে একটা SVG `<svg>` স্ট্রিং বানায়, `{id, success, svg, widthPx, heightPx, lines}` হিসেবে রিটার্ন করে।
3. **dompdf ইনলাইন SVG data URI নেটিভলি সাপোর্ট করে, full vector quality-তে** (`<img src="data:image/svg+xml;base64,...">` — সরাসরি টেস্ট করে কনফার্ম করা হয়েছে) — তাই কোনো rasterization/PNG স্টেপ লাগে না (আলাদা `sharp`/Imagick ডিপেন্ডেন্সি ছাড়াই), এবং কোনো DPI trade-off নাই (vector, যেকোনো প্রিন্ট রেজোলিউশনে ঠিক থাকবে)।
4. **`app/Services/BengaliShapingService.php`** — PHP সাইড wrapper। একটা পুরো PDF render-এর **সব Bengali field-এর shaping job ONE ব্যাচে** পাঠায় (Symfony Process দিয়ে `node shape.mjs` কল করে, stdin দিয়ে JSON payload, stdout থেকে JSON রেজাল্ট পার্স করে) — প্রতি-ফিল্ড আলাদা Node process spawn করলে অনেক স্লো হতো (measured: ~350-400ms Node cold-start + WASM init, ব্যাচ করলে পুরো ডকুমেন্টে এই খরচ **একবারই**)। **কখনো PDF জেনারেশন ব্লক/ক্র্যাশ করতে পারবে না** — Node অনুপস্থিত থাকলে, timeout হলে, বা কোনো নির্দিষ্ট job ফেইল করলে, শুধু সেই field-এর রেজাল্ট অনুপস্থিত থাকে, caller-কে **পুরনো `reorderBengaliMatras()` প্লেইন-টেক্সট ফলব্যাকে** যেতে হয় (নিচে দেখো) — try/catch + null-coalescing দিয়ে গ্যারান্টিড।

**⚠️ ক্রিটিক্যাল বাগ যেটা প্রথম টেস্টে ধরা পড়েছিল (মনে রাখা জরুরি ভবিষ্যতে extend করার সময়):** শেপিং জব-এ **কখনোই `reorderBengaliMatras()`-এর আউটপুট পাঠানো যাবে না** — সেই regex reorder dompdf-এর জন্য raw Unicode স্টোরেজ-অর্ডার ম্যানুয়ালি বদলে দেয়, কিন্তু HarfBuzz নিজেই সঠিক reordering করে (সেটাই তো পুরো পয়েন্ট)। আগে-থেকে-reorder-করা (ভুল) টেক্সট HarfBuzz-এ পাঠালে সেটা ভুল ইনপুট হিসেবে শেপ হয়ে **তৃতীয় একটা ভুল অর্ডার** তৈরি করে (টেস্টে "মনিরুজ্জামান" → "মিনরুজ্জামান" হয়ে গিয়েছিল প্রথম অ্যাটেম্পটে)। **সবসময় raw/original টেক্সট (DB থেকে সরাসরি, `$order->customer_name` ইত্যাদি) শেপ করতে হবে** — `reorderBengaliMatras()`-এর আউটপুট শুধু plain-text fallback-এর জন্য।

**Integration (`WaybillPdfService` + `OrderInvoicePdfService`):**
- দুটো সার্ভিসই constructor-এ `BengaliShapingService` ইনজেক্ট করে (`app(WaybillPdfService::class)`/method-injection দিয়ে রিজলভ করতে হয়, `new WaybillPdfService()` ম্যানুয়াল ইনস্ট্যান্শিয়েশন আর কাজ করবে না — `CourierController`-এর দুটো কল-সাইট আপডেট করা হয়েছে)।
- প্রতিটা Bengali-ধারণকারী ফিল্ডের জন্য (`preg_match('/\p{Bengali}/u', ...)` দিয়ে চেক) `queueShapingJob()` একটা job রেজিস্টার করে raw text দিয়ে, এবং fallback হিসেবে `reorderBengaliMatras()`-করা টেক্সট label array-তে রাখা থাকে যথারীতি। সব label বিল্ড হওয়ার পর **একবার** `shapeBatch()` কল হয়, রেজাল্ট label array-তে `xxxImg`/`xxxImgW`(mm)/`xxxImgH`(mm) হিসেবে অ্যাটাচ হয়।
- Blade-এ প্রতিটা শেপড ফিল্ডের জন্য প্যাটার্ন: `@if($label['xxxImg']) <img src="{{ $label['xxxImg'] }}" style="width:...mm;height:...mm;"> @else <div class="i18n">{{ $label['xxx'] }}</div> @endif` — শেপিং ফেইল করলে/অনুপস্থিত থাকলে ঠিক আগের মতোই রেন্ডার হয়, কোনো ভিজ্যুয়াল রিগ্রেশন নাই।
- **স্কোপ (2026-08-12-এ সম্পূর্ণ ২২টা টেমপ্লেটে extend করা হয়েছে):** শুরুতে শুধু waybill-এর `classic` টেমপ্লেট + `OrderInvoicePdfService`-এ wire করা হয়েছিল (ইউজারের নির্দিষ্ট অভিযোগ অনুযায়ী), তারপর একই সেশনে বাকি ২১টা sticker টেমপ্লেটেও (pathao + ২০টা নতুন) একই প্যাটার্ন extend করা হয়েছে। এখন `WaybillPdfService::resolveShapingSpec()`-এ সবগুলো ২২টা টেমপ্লেট key-এর জন্য field/font-size/wrap-width স্পেক আছে, আর প্রতিটা corresponding Blade partial-এ img/fallback markup যোগ করা আছে। কোনো টেমপ্লেট আর পুরনো regex-reorder-কে প্রাইমারি রেন্ডারিং পাথ হিসেবে ব্যবহার করে না — শুধু Node/শেপিং ব্যর্থ হলে graceful fallback হিসেবে থেকে গেছে।

**নতুন টেমপ্লেট যোগ করলে (ভবিষ্যতে):** প্রতিটা টেমপ্লেটের জন্য (ক) সেই টেমপ্লেটের geometry-তে `contentWidthMm` (বা সমতুল্য wrap-width) আছে কিনা চেক/যোগ করা, (খ) `resolveShapingSpec()`-এ সেই টেমপ্লেটের Bengali ফিল্ডগুলোর স্পেক (px/bold/wrap/color) যোগ করা, (গ) সংশ্লিষ্ট Blade partial-এ img/fallback প্যাটার্ন যোগ করা — মনে রাখা: `<img style="display:block">` একা dompdf-এ সবসময় line-break গ্যারান্টি করে না, ঝুঁকিপূর্ণ জায়গায় explicit `<br>` যোগ করো (দেখো নিচে "dompdf display:block quirk")। `classic`/`pathao` টেমপ্লেটের ডিফ-ই রেফারেন্স ইমপ্লিমেন্টেশন।

**dompdf `display:block` quirk (2026-08-12 আবিষ্কার):** `<img style="display:block">`-এর পরে বেয়ার inline টেক্সট বা আরেকটা `<img>` থাকলে dompdf সবসময় line-break ফোর্স করে না (`invoice_table.blade.php`-এ ধরা পড়েছিল — customerName image আর ফোন নাম্বার একই লাইনে রেন্ডার হচ্ছিল)। ফিক্স প্যাটার্ন: img-এর ঠিক পরে explicit `<br>` যোগ করো, `display:block`-এর উপর একা নির্ভর কোরো না।

**পারফরম্যান্স:** পুরো ডকুমেন্টে (যত orders/fields-ই থাকুক) একটাই Node ইনভোকেশন — measured ~350-400ms (বেশিরভাগ Node startup + WASM init)। PDF ডাউনলোডের জন্য অলক্ষণীয়।

**নতুন প্রোডাকশন ডিপেন্ডেন্সি — ডিপ্লয়মেন্ট নোট:**
- সার্ভারে `node`/`npm` লাগবে (এই সার্ভারে আগে থেকেই আছে, v22)।
- `resources/bengali-shaper/`-এ `npm install` চালাতে হবে (একবার — `node_modules` git-ignored, প্রতিটা নতুন এনভায়রনমেন্টে/ফ্রেশ ক্লোনে লাগবে)।
- PHP-FPM যে ইউজারে চলে (`www-data`) তার `node` এক্সিকিউট করার এবং `resources/bengali-shaper/node_modules/` পড়ার পারমিশন থাকা লাগবে — `sudo -n -u www-data node --version` + সরাসরি `shape.mjs` কল করে ভেরিফাই করা হয়েছে, কাজ করে।
- Node/শেপিং ব্যর্থ হলে PDF জেনারেশন **কখনো ফেইল করবে না** (§৪.৭-এর "কখনো ব্লক/ক্র্যাশ করতে পারবে না" পয়েন্ট) — শুধু plain-text fallback-এ চলে যাবে, চুপচাপ।
...
**✅ ৮.১ Payment History টেবিল যোগ হয়েছে (2026-08-17)** — ম্যানুয়াল পেমেন্ট কালেকশন ফিচারের অংশ, বিস্তারিত [`manual_payment_collection_context.md`](manual_payment_collection_context.md)-এ। `OrderInvoicePdfService::render()` এখন `payments.collector:id,name` eager-load করে, নতুন "PAYMENT HISTORY" টেবিল (Date/Type/Method/Amount/Discount/Received by) + Paid/Extra Discount/Due summary যোগ করে — শুধু `$payments->isNotEmpty()` হলে রেন্ডার হয় (কোনো ম্যানুয়াল পেমেন্ট না থাকা অর্ডারে কোনো পরিবর্তন নেই)। কালেক্টর নাম শুধু Bengali হলে shape হয় (§৪.৭-এর একই প্যাটার্ন); Date/Type/Method/Amount/Discount/Received-by কলাম হেডার ও পেমেন্ট purpose/method-এর লেবেল **ইংরেজিতেই থাকে** — Subtotal/Discount/Total-এর মতো static লেবেলগুলোর existing convention অনুসরণ করে, শুধু dynamic টেক্সট (নাম/ঠিকানা/আইটেম) real shaping পায়।
...
| বাংলা matra reordering হেল্পার (plain-text fallback path-এ ব্যবহৃত, §৪.৫) | `WaybillPdfService::reorderBengaliMatras()` + `OrderInvoicePdfService::reorderBengaliMatras()` (ডুপ্লিকেট, শেয়ার্ড ক্লাসে এক্সট্র্যাক্ট করা হয়নি) |
| ✅ Real বাংলা text shaping (HarfBuzz, §৪.৭ — mainline path) | `app/Services/BengaliShapingService.php`, `resources/bengali-shaper/shape.mjs` |
