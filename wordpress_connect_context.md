...
শেষ আপডেট: ২০২৬-০৮-২৮ — **`SETUP.md` sync।** pre_launch_polish_context.md §ট ধরতে গিয়ে দেখা গেছে `SETUP.md`-এর "Online payment gateways" QA চেকলিস্ট শুধু v1.19.0/1.19.2 পর্যন্ত কভার করত, যদিও প্লাগইনের আসল ভার্সন v1.19.6 (changelog.md-এ সব ঠিকই ছিল, শুধু SETUP.md-এর টেস্ট-চেকলিস্টে ফিরে আসেনি v1.19.3-এর status panel, v1.19.5-এর known-quirks, v1.19.6-এর shortcut form)। এখন সব যোগ হয়েছে — ব্যবহারকারীর আসল WooCommerce স্টেজিং QA পাস (যেটা এখনো বাকি, dev environment-এ সম্ভব না) যেন এই already-known/fixed জিনিসগুলো আবার নতুন বাগ হিসেবে আবিষ্কার না করে।

শেষ আপডেট: ২০২৬-০৮-২০ (২) — **v1.19.6 — BSOL Connect-এর নিজের Settings-এই enable/title শর্টকাট।** সেলার জিজ্ঞেস করলেন গেটওয়ে enable/disable + title-এর জন্য কেন WooCommerce → Settings → Payments-এ (গেটওয়ে-প্রতি আলাদা স্ক্রিন) যেতে হয়, BSOL Connect-এর নিজের পেজ থেকে কেন না। Channel এর অস্তিত্ব/ক্রেডেনশিয়াল এখনো BSOL dashboard-এই থাকে (অপরিবর্তিত) — কিন্তু enable/title আসলে WooCommerce-নেটিভ ফিল্ড (`woocommerce_bsol_{provider}_settings` অপশনেই সেভ হয়, `Bsol_Gateway` স্ট্যান্ডার্ড `WC_Settings_API::get_option()` দিয়েই পড়ে) — তাই BSOL Connect → Settings → Payment Gateways প্যানেলে নতুন এক-পাতার টেবিল (`render_gateway_shortcut_form()`) যোগ হয়েছে যেটা সরাসরি সেই একই অপশনে লেখে, কোনো নতুন ডেটা মডেল না, কোনো cache delay ছাড়াই সাথে সাথে effective হয়। বিস্তারিত §১২-এর নিচে।
...
### সংযোজন (v1.19.6, পরের দিন — enable/title শর্টকাট)

উপরের "BSOL: prefix" আর "COD-তে টেক্সট নেই" দুটো সমস্যাই দেখার পর সেলার প্রশ্ন করলেন — enable/disable + title বদলাতে কেন WooCommerce → Settings → Payments-এ গেটওয়ে-প্রতি আলাদা স্ক্রিনে যেতে হয়, BSOL Connect-এর নিজের পেজ থেকে কেন না।

**সিদ্ধান্ত**: channel-এর অস্তিত্ব/ক্রেডেনশিয়াল BSOL dashboard-এই থাকবে (অপরিবর্তিত — কুরিয়ার ক্রেডেনশিয়াল আর checkout-OTP টগলের মতোই একই নীতি), কিন্তু enable/disable আর title আসলে WooCommerce-নেটিভ ফিল্ড — `Bsol_Gateway` এগুলো স্ট্যান্ডার্ড `WC_Settings_API::get_option()` দিয়েই পড়ে, `woocommerce_bsol_{provider}_settings` অপশনে সেভ হয়, ঠিক যেভাবে WooCommerce-এর নিজের সেটিংস স্ক্রিন সেভ করত। তাই এটা সরিয়ে আনার প্রশ্ন না, শুধু একটা দ্রুত রাস্তা দেওয়ার প্রশ্ন — দুই জায়গাতেই একই ডেটা।

**যোগ হলো**: BSOL Connect → Settings → Payment Gateways প্যানেলে নতুন এক-পাতার টেবিল (`Bsol_Admin::render_gateway_shortcut_form()`) — প্রতিটা বর্তমানে-দৃশ্যমান চ্যানেলের জন্য একটা row (Enabled checkbox + Title text field), Save করলে `handle_gateway_shortcut_save()` সরাসরি সেই একই `woocommerce_bsol_{provider}_settings` অপশনে লেখে। কোনো নতুন ডেটা মডেল/মাইগ্রেশন না। Provider নাম একটা fixed whitelist (`PROVIDER_LABELS`) দিয়ে ভ্যালিডেট করা হয় লেখার আগে — raw POST থেকে সরাসরি `wp_options` key বানানো হয় না।

এই ফর্মের পরিবর্তন **সাথে সাথে effective হয়** (`get_option()` প্রতি গেটওয়ে ইনস্ট্যানশিয়েশনে লাইভ পড়া হয়, আলাদা কোনো cache নেই) — উপরের channel *লিস্ট*-এর মতো ১৫ মিনিটের delay নেই এখানে (channel list-এর delay এখনো আছে, কারণ সেটা নির্ধারণ করে কোন গেটওয়ে আদৌ register হবে কিনা, শুধু title/enabled না)।

খালি title সাবমিট করলে blank সেভ হয় না — প্লেইন provider label-এ fallback করে (ঠিক আগের "Cash on delivery" blank-title রিপোর্টটাই মনে করিয়ে দিল কেন এটা দরকার)।
