...
// ---------------------------------------------------------------------------
// Page content — mounted by app/page.tsx (bn, at "/") and app/en/page.tsx
// (en, at "/en"), each a thin Server Component supplying its own metadata/
// JSON-LD. `initialLocale` is the URL's own language, not a guess from
// localStorage — the previous version defaulted this to a plain useState
// and only corrected it after mount from getStoredLocale(), which meant the
// server-rendered HTML a crawler's first pass saw never matched the URL's
// actual language (seo_context.md).
// ---------------------------------------------------------------------------
      ...
      <header className="sticky top-0 z-30 border-b border-[var(--border)] bg-[var(--background)]/85 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={locale === "bn" ? "/en" : "/"}
              onClick={() => localStorage.setItem(LOCALE_STORAGE_KEY, locale === "bn" ? "en" : "bn")}
