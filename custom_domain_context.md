...
- **`legacy_slug`** — সাবডোমেইনের আগেকার প্রতিটি পেজের পুরনো platform URL (`bsol.{apex}/lp/{slug}`) সংরক্ষণ করে। migration একবার backfill করে (১২টা পেজ, শূন্য অমিল), অ্যাপ কখনো লেখে না, `$fillable`-এ নেই। নতুন পেজে এটা `null` — তারা শুধু সেলারের সাবডোমেইনে থাকে, ফলে `/lp/{slug}` চিরকাল দ্ব্যর্থহীন।
  - **কেন দরকার ছিল:** সিদ্ধান্তের সময় প্রকাশিত পেজগুলোতে ২১২টি ভিজিট ও ২৩টি বাস্তব অর্ডার ছিল — `/lp/` লিংক জীবিত, ভাঙা যাবে না। অনুমান নয়, ডেটা দেখে যাচাই করা।
- **`LandingPageResolver`** — প্রতিটি পাবলিক ল্যান্ডিং endpoint এখানেই যায়, রিকোয়েস্টের Host থেকে শপ বের করে। প্ল্যাটফর্ম host → `legacy_slug`; সেলার সাবডোমেইন → ওই শপের `slug`; **মালিকবিহীন সাবডোমেইন → কিছুই না** (নাহলে `unknown.{apex}/offer` অন্য সেলারের পেজ দেখাত)। ফ্রন্টএন্ডে কোনো পরিবর্তন লাগেনি, কারণ পাবলিক কলগুলো আগে থেকেই same-origin relative।
...
- **`/lp/{slug}` এখন canonical ঠিকানায় স্থায়ী রিডাইরেক্ট করে** (যদি শপের সাবডোমেইন থাকে) — নাহলে একই পেজ দুই ঠিকানায় 200 দিত: duplicate content, এবং প্ল্যাটফর্ম URL-টা এমন ডোমেইনে যেটা সেলার Meta-তে verify করতে পারে না। সাবডোমেইনহীন শপের পেজ আগের মতোই `/lp/`-এ সরাসরি সার্ভ হয়। proxy-র `x-bsol-shop-subdomain` হেডার দিয়ে দুটো কেস আলাদা করা হয়, তাই রিরাইট করা রিকোয়েস্টে লুপ হয় না।
