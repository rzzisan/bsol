...
এই ফিচার `tracking_capi_context.md`-এর **T8a** — সেটার §8.3 (কেস C2)। **সম্পন্ন**, এবং এর ফলে ট্র্যাকিং পরিকল্পনার একটা বড় অংশ বাতিল হয়ে গেছে (ওই ডকের কেস B এবং §11-এর অর্ধেক সিদ্ধান্ত — ২০২৬-০৮-১৫-এ পুরো ডক গুছিয়ে লেখা হয়েছে):

- `_fbp`/`_fbc` কুকি exact-host-এ থাকায় ক্রস-সেলার দূষণ শেষ — শেয়ার্ড ডোমেইনের সবচেয়ে বড় correctness bug (`tracking_capi_context.md §8.2`) কাঠামোগতভাবে সমাধান। ফলে সেলার সাবডোমেইনে **browser Pixel + server CAPI দুটোই চলবে (Full tracking)**।
...
- ⚠️ **একই apex রাখায় শেয়ার্ড রেপুটেশন ঝুঁকি বহাল** — ট্র্যাকিং ডকের §8.6 আলাদা apex সুপারিশ করেছিল; ঝুঁকি ও প্রশমন সেখানে লেখা।
- **Meta domain verification — যাচাই হয়ে গেছে (২০২৬-০৮-১৫): সেলার নিজে পারবে না।** Meta শুধু **root ডোমেইন** verify করতে দেয়, সাবডোমেইন নয়, আর একটা root ডোমেইন **একটাই business**-এ থাকতে পারে (`tracking_capi_context.md §11.2`)। ফলে:
  ...
  - ✅ **সেলারের বিজ্ঞাপন আটকাবে না** — Meta-র নিজের ডক অনুযায়ী verification নিয়ন্ত্রণ করে *"who can edit link descriptions on ads pointing to your domain"*, অর্থাৎ **লিংক-প্রিভিউ সম্পাদনা, বিজ্ঞাপন চালানো নয়**। তাই per-seller partner-assignment লাগবে না; শুধু কোনো সেলার নিজের সাবডোমেইনের লিংক-প্রিভিউ টেক্সট কাস্টমাইজ করতে চাইলে তখন তাকে partner করা যাবে।
  - ✅ **AEM আর কোনো ঘাটতি নয়** — Meta ২০২৫-এর জুনে web ইভেন্টের ম্যানুয়াল AEM কনফিগারেশন (৮-ইভেন্ট র‍্যাংকিং) তুলে দিয়েছে, এখন স্বয়ংক্রিয়। তাই "সেলার নিজের ইভেন্ট প্রায়োরিটি সেট করতে পারে না" কথাটার আর অর্থ নেই — কনফিগার করার মতো কিছু **কারও** হাতেই নেই, নিজের ডোমেইনওয়ালা সেলারেরও নয়।
  - **সারমর্ম: সাবডোমেইনে বিজ্ঞাপন চালানোয় ট্র্যাকিং-এর কোনো বাস্তব সীমাবদ্ধতা নেই।** একমাত্র প্রকৃত পার্থক্য **শেয়ার্ড রেপুটেশন** (`tracking_capi_context.md §8.6`) — এবং সেটাই এখন T8b-র মূল যুক্তি, AEM নয়।
