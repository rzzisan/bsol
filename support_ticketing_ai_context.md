...
## লাইভ চ্যাট থেকে "টিকেট খুলব?" জিজ্ঞেস করে টিকেট ওপেন করা (added 2026-08-30)

আগে লাইভ চ্যাটে AI সমাধান দিতে না পারলে চুপচাপ `escalate_to_admin` কল করে দিত (কনভারসেশনের `admin_unread_count` বাড়িয়ে)। এখন — সাধারণ (জরুরি না এমন) সমস্যায় — সরাসরি escalate করার বদলে সেলারকে জিজ্ঞেস করে: "এটার জন্য আমাদের টিম বিস্তারিত দেখলে ভালো হবে — আপনার জন্য কি একটা সাপোর্ট টিকেট খুলে দেব?" সেলার হ্যাঁ বললে (পরের মেসেজে) আসল টিকেট খুলে দেয়, পুরো চ্যাট-ইতিহাস কপি করে, admin-এর জন্য escalated flag করে।

- নতুন টুল **`open_support_ticket`** (`subject`, `category` ইনপুট) — **শুধু লাইভ চ্যাটে পাওয়া যায়, টিকিটে না** (টিকিটের ভেতর থেকে আরেকটা টিকিট খোলার কোনো মানে নেই) — `toolDefinitions(bool $includeOpenTicket)` কন্ডিশনালি এটা যোগ করে, `respondToConversation()` তখনই `onOpenTicket` closure পাস করে, `respondToTicket()` করে না।
- `AiSupportAgentService::openTicketFromConversation()` — নতুন `SupportTicket` তৈরি করে (`TKT-XXXXXX` নম্বর, `escalated=true`), তারপর সেই কনভারসেশনের **পুরো মেসেজ হিস্টরি** (user/admin/ai সব sender_type সহ) নতুন টিকেটে কপি করে — admin-এর কাছে পুরো প্রেক্ষাপট থাকে।
- System prompt-এ একটা নতুন **শুধু-লাইভ-চ্যাট** সেকশন (`systemPrompt($settings, bool $isLiveChat)`) — রিফান্ড/অ্যাকাউন্ট-পরিবর্তনের মতো জরুরি/স্পর্শকাতর বিষয়ে আগের মতোই সরাসরি escalate_to_admin কল করে (জিজ্ঞেস করার দরকার নেই), শুধু সাধারণ "সমাধান দিতে পারছি না" পরিস্থিতিতে আগে জিজ্ঞেস করে।

লাইভ verify করা হয়েছে আসল মডেল দিয়ে (Gemini, যেহেতু টেস্টিং করতে করতে Groq-এর দুটো key-ই সাময়িক TPM cooldown-এ চলে গিয়েছিল) — একটা প্রশ্ন যেটার জন্য কোনো tool/knowledge-base কভারেজ নেই (কাস্টম CSS) দিয়ে দেখা গেছে: turn ১-এ AI ঠিকই জিজ্ঞেস করেছে টিকেট খুলবে কিনা, turn ২-এ "hae, ticket khule dao" বলাতে আসল `TKT-000019` টিকেট খুলে গেছে সঠিক subject/category সহ, আর পুরো ২-টার্নের কথোপকথন টিকেটে কপি হয়ে গেছে।

**সাথে সাথে একটা রিয়েল প্রি-এক্সিস্টিং বাগ ধরা পড়েছে ও ফিক্স হয়েছে:** `support_messages.sender_id` কলামটা `NOT NULL` ছিল, অথচ `AiSupportAgentService`-এর AI রিপ্লাই লেখার কোড শুরু থেকেই `sender_id = null` দিয়ে লিখত (AI-এর তো কোনো user row নেই) — মানে **লাইভ চ্যাট widget-এ AI রিপ্লাই কখনো আসলে DB এরর দিয়ে ব্যর্থ হতো**, এই বাগ এতদিন ধরা পড়েনি কারণ এই সেশনের সব লাইভ টেস্টিং টিকিটে হয়েছিল (`support_ticket_messages.sender_id` শুরু থেকেই nullable ছিল), লাইভ চ্যাটে কখনো আসলে টেস্ট করা হয়নি। নতুন migration দিয়ে `support_messages.sender_id`-ও nullable করা হয়েছে।
