...
**S4 + S6 ✅ লাইভ ও visually ভেরিফাইড (২০২৬-০৮-২১, user-এর অনুরোধে S2/S3/S5 বাদ দিয়ে):** localStorage cart (`CartProvider`/`useCart`, মিক্সড ফিজিক্যাল+ডিজিটাল কার্ট client-side-ই ব্লক করে), `/store/category/{slug}`, `/store/product/{slug}` (গ্যালারি, ৫-বাটন অ্যাকশন রো + Call/WhatsApp/Messenger, ৬-ট্যাব + collapsible grouped spec, right-rail sidebar), `/store/search` (browse-all + ফিল্টার/সর্ট), `/store/cart`। একটা real bug ব্রাউজার-টেস্টে ধরা পড়েছে — client fetch হেল্পার ভুলবশত absolute `NEXT_PUBLIC_API_BASE_URL` ব্যবহার করছিল (platform host-এ hit করত, "কোনো শপ নেই" ধরে ৪০৪), রিলেটিভ `/api`-এ ফিক্স। বিস্তারিত `seller_storefront_context.md §১৬`। বাকি S2/S3/S5/S7-S9 (dashboard ফিল্ড এডিটর, checkout backend, ফুল হোমপেজ থিম, রিভিউ, SEO, ট্র্যাকিং)।
