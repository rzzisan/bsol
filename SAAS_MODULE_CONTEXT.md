...
Last updated: 2026-08-23 — **SaaS বিলিং/সাবস্ক্রিপশন redesign শুরু (user-এর নিজের আইডিয়া, নতুন §24)** — ধাপ ১ (package feature-gate ফাউন্ডেশন) ✅ সম্পন্ন ও লাইভ। বিস্তারিত §24 ও `subscription_billing_context.md §৯`। Older entries kept as-is:

Last updated: 2026-08-20 (২) — **WhatsApp Business integration Phase 1 লাইভ** — `feature_roadmap_context.md` আইটেম #২। Order-status automation (Meta-approved টেমপ্লেট) + ২-way ইনবক্স, Facebook Page/Messenger-এর architecture সরাসরি reuse করে (webhook shared callback URL, `wa_id`-ভিত্তিক exact customer auto-link)। ⚠️ `whatsapp_business_messaging` App Review এখনো বাকি — real কাস্টমারদের জন্য এখনো ব্লকড (Facebook CAPI-র মতোই "কোড শেষ, review বাকি" অবস্থা)। টেস্ট লেখার সময় একটা real Postgres transaction-poisoning বাগ পাওয়া গেছে ও ফিক্স হয়েছে (savepoint দিয়ে)। বিস্তারিত `whatsapp_context.md`। Older entries kept as-is:

Last updated: 2026-08-20 — **SMS credit auto-recharge (Auto-top-up Phase 1) লাইভ** — `feature_roadmap_context.md` আইটেম #৩। সেলার bKash Agreement কানেক্ট করে থ্রেশহোল্ড/top-up amount সেট করলে balance কমে গেলেই নিজে থেকে রিচার্জ হয়, ৩ বার consecutive ব্যর্থ হলে circuit-breaker auto-disable করে দেয়। `BkashPaymentGatewayClient`-এ নতুন Agreement মেথড (create/execute/charge/cancel), নতুন `saved_payment_methods` টেবিল, `AutoRechargeSmsCreditJob`। ⚠️ bKash Agreement API-র shape sandbox-এ verify করা হয়নি এখনো। Subscription auto-renew ইচ্ছাকৃতভাবে deferred। বিস্তারিত `auto_top_up_context.md`। Older entries kept as-is:
...
| Order Management | ✅ DONE | `OrderController.php` (CRUD, stats, bulk-status, create-bootstrap), `OrderStatusService.php`, `OrderBulkImportController`+`OrderBulkImportService` (CSV bulk import, §23) | Status change একটা centralized service দিয়ে হয় যেটা SMS automation + accounting + fraud check তিনটাই trigger করে — এটা §14.2-এর "Event/Listener" আইটেমের কার্যকর বিকল্প (আলাদা Laravel Event/Listener লেখা হয়নি, কিন্তু একই কাজ হচ্ছে) |
...
গবেষণায় পাওয়া গেছে অনেক প্রয়োজনীয় ইনফ্রা আগে থেকেই কোডবেসে আছে ও সরাসরি reuse করা যাবে: admin-controlled global file-policy প্যাটার্ন (`ProductMediaSetting`/`Admin\ProductMediaSettingsController`, `effectivePolicy()`), email+SMS dual-channel dispatch (`NotificationDispatchService` + `EmailConfiguration`/`NotificationTemplate`/`NotificationUseCaseBinding` — এটা আসলে সত্যিকারের PHPMailer দিয়ে ইমেইল পাঠায়, নতুন বানাতে হবে না), secure timing-safe token-link প্যাটার্ন (`Order.public_token` + `hash_equals`), এবং private (non-web-exposed) storage disk আগে থেকেই `'public'` disk থেকে আলাদা কনফিগার করা।
...
**S0 ✅ লাইভ (২০২৬-০৮-২১):** root/হোমপেজ রাউটিং + reserved path। বর্তমান বাগ ফিক্স হয়েছে — সেলারের সাবডোমেইনের bare root এখন আর platform লগইনে redirect করে না, বদলে storefront placeholder home (বা সেলার-নির্বাচিত landing page) দেখায়; `proxy.ts`-এর পুরনো redirect আসলে ইচ্ছাকৃত নিরাপত্তা সিদ্ধান্ত ছিল (login form কম-বিশ্বস্ত origin-এ রেন্ডার না হওয়া, `domain_security_audit.md M-3`) — সেটা ভাঙা হয়নি, `/store`-এ কোনো লগইন ফর্মই নেই বলে নিরাপদে সেখানে রাউট করা গেছে। বিস্তারিত `seller_storefront_context.md §১৪`।

**S1 ✅ লাইভ (২০২৬-০৮-২১):** পাবলিক ক্যাটালগ API (`/public/storefront/home|categories|products|products/{slug}`), allowlist-বিল্ট (raw model dump না)। Product-এ নতুন `slug` (per-shop unique, প্রোডাকশনের ১৩টা প্রোডাক্টে backfilled) + S2-এর কলামও (features/specifications/is_featured ইত্যাদি) একই migration-এ। দুটো real bug ধরা পড়েছে বিল্ডের সময়: `Product.variants` নাম-সংঘর্ষ (legacy jsonb কলাম বনাম HasMany রিলেশন, একই নাম — `getRelation()` দিয়ে ফিক্স, শিপ হওয়ার আগেই ধরা পড়েছে) এবং একটা **pre-existing, এই ফিচারের বাইরের** cost_price leak বিদ্যমান পাবলিক ভ্যারিয়েন্ট-রিজলভ এন্ডপয়েন্টে (`ProductVariantFormatter::format()` reuse করায়) — ইনলাইন ফিক্স না করে `spawn_task` দিয়ে flag করা হয়েছে। বিস্তারিত `seller_storefront_context.md §১৫`।

**S4 + S6 ✅ লাইভ ও visually ভেরিফাইড (২০২৬-০৮-২১, user-এর অনুরোধে S2/S3/S5 বাদ দিয়ে):** localStorage cart (`CartProvider`/`useCart`, মিক্সড ফিজিক্যাল+ডিজিটাল কার্ট client-side-ই ব্লক করে), `/store/category/{slug}`, `/store/product/{slug}` (গ্যালারি, ৫-বাটন অ্যাকশন রো + Call/WhatsApp/Messenger, ৬-ট্যাব + collapsible grouped spec, right-rail sidebar), `/store/search` (browse-all + ফিল্টার/সর্ট), `/store/cart`। একটা real bug ব্রাউজার-টেস্টে ধরা পড়েছে — client fetch হেল্পার ভুলবশত absolute `NEXT_PUBLIC_API_BASE_URL` ব্যবহার করছিল (platform host-এ hit করত, "কোনো শপ নেই" ধরে ৪০৪), রিলেটিভ `/api`-এ ফিক্স। বিস্তারিত `seller_storefront_context.md §১৬`।

**S2 ✅ লাইভ:** dashboard প্রোডাক্ট পেজে "স্টোরফ্রন্ট" সেকশন — show_in_storefront/is_featured, Key Features bullet-list এডিটর, গ্রুপড Specification টেবিল এডিটর, seo_content/warranty_override/delivery_override — ব্যাকএন্ড round-trip + পাবলিক পেজে সাথে সাথে প্রতিফলন ব্রাউজারে ভেরিফাই করা।

**S3 ✅ লাইভ (COD-only স্কোপে, `StorefrontCheckoutController`-এর class docblock-এ ডকুমেন্টেড সিদ্ধান্ত):** `POST /public/storefront/orders` (`StorefrontOrderService`, `LandingPageOrderService`-এর ছোট duplicate), `GET /public/storefront/orders/{token}`, `/store/checkout` + `/store/order/[token]` ফ্রন্টএন্ড পেজ। অনলাইন পেমেন্ট (তাই ডিজিটাল প্রোডাক্ট চেকআউট) এই পাসে না — landing-page-slug-bound `OnlinePaymentController` জেনারালাইজ করা আলাদা কাজ (S3b)। একটা real order (`ORD-20260821-0006`) `zareen.zyrotechbd.com`-এ পূর্ণ ব্রাউজার ফ্লো দিয়ে তৈরি+ভেরিফাই করা হয়েছে (mixed-cart guard, digital-cart guard, Buy Now → checkout → confirmation, সব কাজ করেছে)।
...
## 23. Bulk/CSV Order Import — ✅ সম্পন্ন ও লাইভ (২০২৬-০৮-২২)

অডিটের P3 প্রায়োরিটি (user সরাসরি এই আইটেম বেছে নিয়েছেন, P2 skip করে) — `feature_roadmap_context.md` §16.8। সেলার এখন `/dashboard/orders/bulk-import`-এ CSV আপলোড করে একসাথে অনেক অর্ডার তৈরি করতে পারে।

**ফ্লো:** টেমপ্লেট ডাউনলোড → Preview (প্রতিটা সারি ভ্যালিডেট করে, কোনো অর্ডার তৈরি হয় না, ভুল সারির row-number+error দেখায়) → Commit (শুধু সঠিক সারিগুলো একটা `DB::transaction()`-এ তৈরি হয়)। দুটো এন্ডপয়েন্টই stateless — client একই ফাইল দুবার আপলোড করে, সার্ভার-সাইডে কোনো temp-token/ফাইল-ক্যাশ নেই। এক CSV সারি = এক অর্ডার = এক পণ্য (v1 স্কোপ — একাধিক প্রোডাক্টের অর্ডার পরে ম্যানুয়ালি এডিট করে যোগ করতে হবে)।

**আর্কিটেকচার:** নতুন `OrderBulkImportService` — `OrderController::store()`-কে রিফ্যাক্টর না করে ইচ্ছাকৃত ছোট duplicate (`StorefrontOrderService`-এর ডকব্লকের একই যুক্তি — established convention), CSV সারি manual-create-এর চেয়ে সহজ (variant matching নেই, সবসময় ১টা আইটেম) বলে একই core primitive (`Order::generateOrderNumber()`, `Customer::syncFromOrder()`, `AccountingService::onOrderCreated()`) reuse করে আলাদা path হিসেবে। মাসিক অর্ডার-লিমিট respect করে (কোটার বাইরের সারি "skipped_for_quota" হিসেবে রিপোর্ট হয়, silently drop হয় না)। Pattern A (`staff_permission:orders`) — বিদ্যমান order-creation permission-ই reuse।

**🔴 লাইভ ভেরিফিকেশনের সময় আবিষ্কৃত ও ফিক্সড platform-wide বাগ:** `orders.order_number`-এ একটা **global** unique DB constraint ছিল, অথচ `Order::generateOrderNumber()` সবসময়ই per-shop scoped সিকোয়েন্স জেনারেট করে (`whereIn('user_id', $shopUserIds)`) — মানে দুইটা ভিন্ন শপ একই দিনে দুজনেই তাদের প্রথম অর্ডার তৈরি করলে দুজনেই "ORD-{date}-0001" পেত, আর দ্বিতীয়জনের INSERT `SQLSTATE 23505` দিয়ে 500 error দিত। এটা শুধু bulk import-এর বাগ না — `OrderController::store()`, `StorefrontOrderService`, `LandingPageOrderService`, `Connect\ConnectOrderController` সবগুলো path-ই সমানভাবে এফেক্টেড ছিল, যেকোনো সময় randomly যেকোনো সেলারের প্রথম অর্ডারে ঘটতে পারত। মাইগ্রেশন `2026_08_22_180000_scope_order_number_unique_per_shop.php`: constraint global → `unique(['user_id', 'order_number'])`, ঠিক products/landing_pages-এর `slug` কলামের একই established প্যাটার্ন (per-shop unique, global না)। `order_number` কখনো public lookup key হিসেবে ব্যবহৃত হয় না (সেটা আলাদা globally-unique `public_token`) আর কোডবেসে প্রতিটা `where('order_number', ...)` query আগে থেকেই shop-scoped (grep-verified) — তাই এই কনস্ট্রেইন্ট বদলে কোনো cross-shop lookup ভাঙেনি।

**টেস্ট:** `OrderBulkImportTest.php` — টেমপ্লেট শেপ, preview-এর valid/invalid+row-number+SKU-match, commit শুধু সঠিক সারি তৈরি করে (সঠিক total/order_number), মাসিক কোটা আংশিক ফিল + বাকিটা skipped রিপোর্ট, oversized ফাইল (৫০০+ সারি) reject, non-CSV reject, staff denied। ফুল রিগ্রেশন সুইট গ্রিন (৩টা known baseline failure বাদে)। লাইভ curl ভেরিফিকেশন disposable টেস্ট অ্যাকাউন্ট দিয়ে (তৈরি+cleanup, tinker)।

## 24. SaaS বিলিং/সাবস্ক্রিপশন redesign — Phase 5, ধাপ ১ ✅ (২০২৬-০৮-২৩)

User-এর নিজের আইডিয়া থেকে শুরু হওয়া একটা নতুন উদ্যোগ (audit-এর P-লিস্টের বাইরে) — পূর্ণ প্ল্যান ও প্রায়োরিটি-অর্ডার `subscription_billing_context.md §৯`-এ। স্কোপ: অর্ডার-কোটা redesign (তৈরির সময় ব্লক না করে প্রসেসিং-এর সময় ব্লক), Add-on/credit প্যাকেজ (order credit/landing page/storefront/tracking boost), আর একটা নতুন ফিচার/মডিউল অ্যাক্সেস কন্ট্রোল সিস্টেম — সব বিদ্যমান proration/invoice/bKash-payment ইঞ্জিন (§18, Phase 1-4) রিইউজ করে।

**ধাপ ১ (feature-gate ফাউন্ডেশন) সম্পন্ন:** `subscription_packages.feature_flags` (json, key=>bool) — display-only `features` বুলেট-লিস্ট কলাম থেকে ইচ্ছাকৃতভাবে আলাদা। নতুন `package_feature:{key}` middleware, প্রথম দুটো key `storefront` ও `facebook` (FB tracking+leads) দিয়ে শুরু। **Default-allow** (`false` explicitly সেট না করলে সব প্যাকেজে খোলা থাকে) — এই কোডবেসের বাকি সব প্যাকেজ-লিমিটের (`max_orders` ইত্যাদি) "null = আনরেস্ট্রিক্টেড" কনভেনশনের সাথে সামঞ্জস্যপূর্ণ রাখতে বিল্ডের মাঝপথে সংশোধন করা হয়েছে (মূল প্ল্যানে "default-deny" ছিল)। সব বিদ্যমান production প্যাকেজ grandfathered। Admin প্যাকেজ ফর্মে (`/admin/packages`) checkbox যোগ হয়েছে। বিস্তারিত + verification log: `subscription_billing_context.md §৯.৭`।

**পরবর্তী ধাপ:** Order quota redesign (creation-time ব্লক সরিয়ে `OrderController::updateStatus()`-এ আনা)।
