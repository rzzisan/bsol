...
> **🚨 সেলার সাবডোমেইন — Facebook-সংশ্লিষ্ট প্রভাব (2026-08-15):** সেলার এখন `{label}.zyrotechbd.com`-এ নিজের ঠিকানা পায় (`custom_domain_context.md`)। দুটো জায়গায় প্রভাব: (১) **Facebook OAuth কলব্যাক** সেলারকে তার নিজের ঠিকানায় ফেরত পাঠায় — ঠিকানা resolve হয় signed `state`-এর `user_id` থেকে, রিকোয়েস্টের Host থেকে নয় (Meta এই রিডাইরেক্ট নিয়ন্ত্রণ করে, তাই Host বিশ্বাস করা যায় না); state decode হওয়ার আগে প্ল্যাটফর্ম URL-ই একমাত্র নিরাপদ গন্তব্য। (২) **CAPI test event-এর `event_source_url`** এখন সেলারের নিজের ঠিকানা। **ভবিষ্যতের জন্য গুরুত্বপূর্ণ:** সেলার এখন নিজের সাবডোমেইন Meta Business Manager-এ verify করতে পারে কিনা তা **এখনো বাস্তবে পরীক্ষা করা হয়নি** — এর উত্তরই ঠিক করবে ল্যান্ডিং পেজে পূর্ণ (browser+CAPI) নাকি শুধু server-side ট্র্যাকিং চলবে (`tracking_capi_context.md §11.7`)।
...
## 8. §6 items 1 + 4 — implemented 2026-08-08

### Item 1 — Multi-page support

`facebook_page_connections` had `unique('user_id')` — the actual blocker. `fb_page_id` stays globally unique (one Page belongs to exactly one BSOL seller); dropped `unique('user_id')`, added a plain index instead. [FacebookConnectController::persistConnection()](backend/app/Http/Controllers/Api/FacebookConnectController.php) now matches `updateOrCreate` on `(user_id, fb_page_id)` instead of `user_id` alone, so connecting a second Page creates a new row instead of overwriting the first.

- `status()` returns all of a seller's connected Pages (was a single object).
- `select()` now takes `page_ids[]` — a seller can connect several Pages from one OAuth consent screen in one action, not one at a time.
- `disconnect()` takes a connection id (route `DELETE /facebook/connect/{id}`) — was a single bodyless DELETE.
- [settings/facebook/page.tsx](frontend/src/app/dashboard/settings/facebook/page.tsx): checkbox multi-select picker, one card per connected Page with its own Disconnect button, "Connect Another Page" button always available.
- [leads/page.tsx](frontend/src/app/dashboard/leads/page.tsx): shows which Page a lead came from, once the seller has more than one connected (small tag next to sender name) — `FacebookLeadController` now eager-loads `pageConnection:id,page_name`.
- **No changes needed** in `FacebookLeadCaptureService` (webhook capture) — it already resolves the connection by `fb_page_id` from the payload, never by `user_id`, so it was multi-page-safe already.

### Item 4 — Conversions API (CAPI) Purchase events

Deliberately **not** built on the existing Page-connect OAuth (`business_management`/`pages_*` scopes) — a Pixel + CAPI access token is a Business Manager / Events Manager asset, unrelated to Page permissions, and would need `ads_management`-class scopes and a fresh App Review round to obtain via OAuth. Instead: the seller pastes their own Pixel ID + CAPI access token (generated by them in Events Manager → Conversions API → "Generate access token") — zero new Meta permissions, works immediately, no review gate.

- New `facebook_pixel_settings` table (per-user, `access_token` encrypted) — separate from `facebook_page_connections` on purpose.
- [FacebookCapiClient](backend/app/Services/Facebook/FacebookCapiClient.php) posts to `graph.facebook.com/{v}/{pixel_id}/events` with the seller's own token.
- [SendFacebookCapiPurchaseEventJob](backend/app/Jobs/SendFacebookCapiPurchaseEventJob.php) — **queued**, not sync (real outbound HTTP call to Meta; queue worker confirmed running per §4). Builds the Purchase event from the order: `ph` (SHA-256 hashed phone), client IP/UA, order total/items, `event_id=order_<id>` (dedup key if the seller ever also adds a client-side Pixel).
- Dispatched from `LandingPageController::publicSubmitOrder()` only — **landing-page checkouts only**, not dashboard-entered orders, since only ad-driven checkouts are real ad conversions to report back to Meta.
...
> **আপডেট (২০২৬-০৮-১৪):** এই CAPI implementation-টা এখন একটা পূর্ণাঙ্গ Tracking Platform-এ সম্প্রসারিত হওয়ার পরিকল্পনায় আছে — browser-side Pixel + server-side CAPI একসাথে, full funnel, order-flow (Delivered/Returned) ইভেন্ট, multi-pixel, event log, এবং প্যাকেজ-ভিত্তিক দৈনিক ইভেন্ট লিমিট। সম্পূর্ণ ডিজাইন: **`tracking_capi_context.md`**। নিচের "out of scope" নোটটা (fbp/fbc, client-side pixel) সেখানে সরাসরি সমাধান করা হয়েছে।
