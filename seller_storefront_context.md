...
## ২৪. Theme Templates addendum — "CareSolution" টেমপ্লেট (২০২৬-০৮-২২, ✅ লাইভ)

**প্রেক্ষাপট:** S0-S9 প্ল্যান সম্পূর্ণ হওয়ার পর ব্যবহারকারী `caresolutionbd.com`-এর ডেস্কটপ+মোবাইল স্ক্রিনশট দিয়ে জানান যে স্টোরফ্রন্টের ডিজাইন "সাদামাটা" এবং সেলার কয়েকটা টেমপ্লেট থেকে বেছে নিতে পারবে এমন একটা সিস্টেম চান, প্রথম নতুন টেমপ্লেট হিসেবে এই রেফারেন্স ডিজাইন। কনফার্ম করা স্কোপ সিদ্ধান্ত (সরাসরি জিজ্ঞেস করে): (ক) Cart পেজের Inside/Outside Dhaka শিপিং সিলেক্টর — real ফিচার (সেলার dashboard-এ রেট সেট করে), (খ) "Track Order" — placeholder (Login/Register-এর প্যাটার্নেই), (গ) Promo Code — বাদ (কুপন সিস্টেম নেই)।

**আর্কিটেকচার — একটাই নতুন সেটিং পুরো টেমপ্লেট সুইচ চালায়:**
- `storefront_settings.theme_template` (`standard`|`caresolution`, ডিফল্ট `standard`) + `shipping_charge_inside_dhaka`/`shipping_charge_outside_dhaka` (nullable decimal, না-সেট করলে `home()` API রেসপন্সে ফলব্যাক ডিফল্ট ৭০/১২০ প্রয়োগ হয়)।
- `StorefrontCheckoutController::submitOrder()`-এ optional `shipping_location` ইনপুট — পাঠালে সেলারের কনফিগার করা রেট server-side রিজলভ করে যেকোনো raw client `shipping_charge` override করে (client-supplied amount trust না করার জন্য)।
- ফ্রন্টএন্ডে নতুন `StorefrontThemeProvider`/`useStorefrontTheme()` (ঠিক `StorefrontTrackingProvider`-এর প্যাটার্ন) — `layout.tsx`-এ একবার `home.theme_template` থেকে সিড হয়, নিচের সব ক্লায়েন্ট কম্পোনেন্টে শেয়ার হয়।
- মাত্র ৩টা জায়গায় প্রকৃত template branch: `layout.tsx` (Shell: `StandardShell` বনাম `CaresolutionShell`), `page.tsx` হোমপেজ বডি (`StandardHome` বনাম `CaresolutionHome`), আর `ProductCard` (থিম caresolution হলে `CaresolutionProductCard` রেন্ডার করে) — ProductCard হোম/ক্যাটাগরি/সার্চ/related-products সব জায়গায় reused, তাই এই একটা branch-ই ৪টা সারফেস রিস্কিন করে দেয়, অন্য কোনো পেজ টাচ করা লাগেনি। Cart পেজও একইভাবে `StandardCartPage`/`CaresolutionCartPage` সুইচ।
- নতুন ফাইল সব `components/storefront/templates/{standard,caresolution}/` — Standard-এর ফাইলগুলো আগের কোডের হুবহু extract (কোনো ভিজ্যুয়াল পরিবর্তন নেই), CareSolution-এর নতুন: `CaresolutionShell`, `CaresolutionHeader` (ডার্ক হেডার + সার্চ + ক্যাটাগরি nav row), `CaresolutionMobileNav` (bottom tab bar + slide-in drawer), `CaresolutionFooter`, `CaresolutionHero` (banner grid + fixed trust-badge strip), `CaresolutionProductCard` (SALE ribbon + Cart/Buy দুই বাটন), `CaresolutionFloatingCart`, `CaresolutionWhatsAppFloat`, `CaresolutionCartPage`, `caresolution-toast-context` (একটাই shared bottom toast, প্রতি-কার্ড না)।
- `storefront-cart.tsx`-এ `shippingLocation` state যোগ (localStorage-এ items-এর সাথেই persist, backward-compatible loader পুরনো items-only shape-ও পড়তে পারে)। Checkout পেজ শুধু থিম caresolution হলেই `shipping_location` পাঠায় — Standard টেমপ্লেটের বিদ্যমান সেলারদের অর্ডারের shipping_charge=0 আচরণ অপরিবর্তিত থাকে।

**স্কোপের বাইরে (স্পষ্টভাবে, silently বাদ দেওয়া না):** প্রোডাক্ট-ডিটেইল পেজ এই পাসে রিস্কিন হয়নি (এই টেমপ্লেটের জন্য কোনো রেফারেন্স স্ক্রিনশট দেওয়া হয়নি — Standard লেআউটেই থাকে caresolution থিমেও)। Track Order/Login placeholder-ই থাকে। Promo Code কোথাও নেই। ট্রাস্ট-ব্যাজ স্ট্রিপের কপি/আইকন ফিক্সড, প্রতি-সেলার এডিটেবল না।

**Dashboard:** Settings → Storefront পেজে নতুন "Design Template" (ভিজ্যুয়াল radio card picker) + "Shipping Charge" (Inside/Outside Dhaka রেট ইনপুট) সেকশন যোগ হয়েছে, homepage-mode সেকশনের উপরে।

**ভেরিফাই:** isolated pgsql schema-তে `StorefrontSettingTest`/`StorefrontCatalogTest`/`StorefrontCheckoutTest` extend করে নতুন টেস্ট (২৭টা, সব pass) + ফুল স্যুট ৪৯১ pass (৩টা known baseline failure অপরিবর্তিত)। Migration প্রোডাকশনে apply, `tsc --noEmit` clean, `deploy-safe.sh` সফল। **লাইভ ব্রাউজার ভেরিফাই** (`zareen.zyrotechbd.com`, ক্রোম এক্সটেনশন এই ফেজে রিকানেক্টেড ছিল) — `theme_template=caresolution` সেট করে ডেস্কটপে হোমপেজ (হেডার/hero/trust-badge/featured categories/category rows/about/footer সব ঠিকভাবে রেন্ডার), cart পেজে Inside/Outside Dhaka রেডিও রিয়েল-টাইমে টোটাল রিক্যালকুলেট করলো (৳500+৳130=৳630 কনফার্ম), এবং একটা রিয়েল test checkout (COD, outside_dhaka) সম্পূর্ণ করে অর্ডার কনফার্মেশন পেজে `মোট ৳630.00` দেখা গেছে — server-side shipping resolution end-to-end কনফার্ম। ভেরিফিকেশন শেষে zareen-এর `theme_template` `standard`-এ ফিরিয়ে দেওয়া হয়েছে (লাইভ সেলারের ডিজাইন অপরিবর্তিত রাখতে) — টেস্ট অর্ডারটা রয়ে গেছে (আগের ফেজগুলোর মতোই)। মোবাইল viewport-এ resize_window টুল আসল রেন্ডার viewport পরিবর্তন করেনি (এই এনভায়রনমেন্টের সীমাবদ্ধতা) — মোবাইল-নির্দিষ্ট চাঙ্ক (bottom nav/drawer, `md:hidden`/`sm:` ব্রেকপয়েন্ট) বিদ্যমান স্টোরফ্রন্ট কোডের একই Tailwind কনভেনশন অনুসরণ করে, তাই আলাদা visual pixel-check ছাড়াই একই আস্থার লেভেলে রাখা হয়েছে — স্বচ্ছভাবে উল্লেখ করা হলো, silently assume করা হয়নি। Dashboard settings পেজের নতুন Design Template/Shipping Charge UI browser-এ visually চেক করা যায়নি (login session ছিল না) — শুধু `tsc --noEmit` ক্লিন কম্পাইল দিয়ে ভেরিফাই করা হয়েছে।

---

## ২৫. CareSolution টেমপ্লেট ফলো-আপ — nav bar color, banner product-link, category thumbnail (২০২৬-০৮-২২, ✅ লাইভ)

**প্রেক্ষাপট:** §24 শিপ হওয়ার পর ব্যবহারকারী দুটো স্ক্রিনশট দিয়ে ৩টা নির্দিষ্ট রিফাইনমেন্ট চান: (ক) ক্যাটাগরি ন্যাভ বার (সব ক্যাটাগরি একসাথে হেডারের নিচে) — ডিজাইন যেন একটা প্রপার নেভবারের মতো হয়, ব্যাকগ্রাউন্ড+টেক্সট কালার সেলার সেট করতে পারবে; (খ) হোমপেজ ব্যানারের প্রতিটাতে একটা প্রোডাক্ট লিংক সেট করার সুযোগ (ক্লিক করলে সেই প্রোডাক্টে যায়); (গ) ফিচারড ক্যাটাগরির প্রতিটাতে একটা থাম্বনেইল ছবি সেট করার সুযোগ, সেট না করলে বর্তমান লেটার-সার্কেল ফলব্যাকই থাকবে।

**Backend (migration `2026_08_22_140000`):**
- `storefront_settings.nav_bg_color`/`nav_text_color` (nullable, ডিফল্ট ফলব্যাক `#111827`/`#ffffff`, `home()`-এ প্রয়োগ হয়) — `StorefrontSettingController::update()`-এ ভ্যালিডেটেড।
- `product_categories.thumbnail_url`/`thumbnail_path` — নতুন `ProductCategoryController::uploadThumbnail()`/`removeThumbnail()` (`POST`/`DELETE /categories/{id}/thumbnail`, `staff_permission:products` গ্রুপে, `StorefrontSettingController::uploadBanner()`-এর একই single-image + *_path প্যাটার্ন)। `StorefrontCatalogController::home()`-এর `featured_categories`-এ `thumbnail_url` যোগ হয়েছে।
- **banner `link_url` ভ্যালিডেশন রিলাক্স করা হয়েছে** — আগে শুধু absolute URL (`url` রুল) গ্রহণ করত, এখন `/product/{slug}`-এর মতো relative in-app পাথও গ্রহণ করে (নতুন `linkUrlRule()` closure — `/` দিয়ে শুরু হলে pass, নাহলে `FILTER_VALIDATE_URL`)। Relative রাখা ইচ্ছাকৃত — ব্যানার যেই হোস্টে (সেলারের সাবডোমেইনে) রেন্ডার হয় তার বিরুদ্ধেই resolve হওয়া দরকার, কোনো হার্ডকোডেড apex ছাড়া।

**Frontend:**
- `CaresolutionHeader`-এর ক্যাটাগরি নেভ রো এখন `home.nav_bg_color`/`nav_text_color` দিয়ে ইনলাইন স্টাইল করা (আগে হার্ডকোডেড সাদা bg/কালো টেক্সট ছিল)।
- `CaresolutionHome`-এর Featured Categories গ্রিড — `c.thumbnail_url` থাকলে ছবি, না থাকলে আগের লেটার-সার্কেল fallback (অপরিবর্তিত)।
- Dashboard Settings → Storefront: নতুন "ক্যাটাগরি ন্যাভ বার" সেকশন (দুটো কালার পিকার + লাইভ প্রিভিউ স্ট্রিপ), Banner সেকশনে ফ্রি-টেক্সট লিংক ইনপুটের বদলে প্রোডাক্ট-সিলেক্ট ড্রপডাউন (সিলেক্ট করলে `/product/{slug}` হিসেবে সেট হয়), Featured Categories সেকশন pill-chip থেকে card-grid-এ রিস্ট্রাকচার করে প্রতিটাতে থাম্বনেইল আপলোড/রিমুভ কন্ট্রোল যোগ।

**ভেরিফাই:** নতুন `ProductCategoryThumbnailTest.php` (২টা) + `StorefrontSettingTest`/`StorefrontCatalogTest` extend (৪টা নতুন) — সব pass, isolated pgsql schema। ফুল স্যুট ৪৯৭ pass (৩টা known baseline failure অপরিবর্তিত)। Migration প্রোডাকশনে apply, `tsc --noEmit` clean, `deploy-safe.sh` সফল। **লাইভ ব্রাউজার ভেরিফাই** (`zareen.zyrotechbd.com`) — nav bar dark-green (`#0f2e28`) কাস্টম কালারে রেন্ডার হয়েছে, "Natural" ক্যাটাগরির থাম্বনেইল লেটার-সার্কেলের বদলে আপলোড করা ছবি দেখিয়েছে, ব্যানারে ক্লিক করলে সঠিকভাবে `/product/nich`-এ নেভিগেট করেছে — তিনটাই কনফার্ম। ভেরিফিকেশনের টেস্ট ডেটা (থাম্বনেইল, extra ব্যানার, nav color, theme_template) পরে মুছে/রিসেট করে zareen-এর আসল স্টেট ফিরিয়ে দেওয়া হয়েছে।
