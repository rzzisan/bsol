...
> **🚨 ল্যান্ডিং পেজ এখন সেলারের নিজের সাবডোমেইনে (2026-08-15) — নিচের সেকশনগুলোতে পুরনো `/lp/{slug}` URL-এর উদাহরণ থেকে যেতে পারে:** ল্যান্ডিং পেজ আর প্ল্যাটফর্ম ডোমেইনে থাকে না। ঠিকানা এখন `https://{seller}.zyrotechbd.com/{slug}` — `/lp/` prefix ছাড়া, এবং **`/lp/` রুট ও `legacy_slug` কলাম সম্পূর্ণ সরিয়ে ফেলা হয়েছে**। এর ফলে এই মডিউলে চারটা জিনিস বদলেছে: (১) **slug এখন shop-প্রতি unique**, globally নয় — দুই সেলার একই `offer` রাখতে পারে; (২) প্রতিটি পাবলিক ল্যান্ডিং endpoint `App\Support\LandingPageResolver` দিয়ে যায়, যা রিকোয়েস্টের Host থেকে শপ resolve করে — নতুন কোনো পাবলিক endpoint লিখলে সেটাও এই resolver ব্যবহার করতে **হবে**, নাহলে ভুল শপের পেজ সার্ভ হতে পারে; (৩) **publish করতে সেলারের সাবডোমেইন থাকা বাধ্যতামূলক** (`error_code: subdomain_required`); (৪) পেজের পাবলিক URL সবসময় `LandingPage::canonicalUrl()` থেকে নিতে হবে — এটা **nullable** (সাবডোমেইনহীন শপে ঠিকানা নেই), আর ড্যাশবোর্ড/admin স্ক্রিন কখনো নিজে URL বানাবে না, API-র `public_url` পড়বে। checkout-এর thank-you ধাপও একই host-এ থাকে, কারণ মাঝপথে ডোমেইন বদলালে `_fbp`/`_fbc` কুকি হারিয়ে ট্র্যাকিং ভাঙে। বিস্তারিত: `custom_domain_context.md §14`, CONTEXT.md §৩২।

> **🚨 Staff/Team role সচেতনতা (2026-08-10 যোগ হয়েছে, এই ফাইল তখনও আপডেট হয়নি বলে নিচের বাকি সেকশনে পুরনো `auth()->id()`-স্টাইল উদাহরণ থাকতে পারে):** `LandingPageController`/`LandingMediaLibraryController`/`LandingPageAnalyticsController`/`AbandonedCheckoutController` — সব এখন staff-scoping-aware (`whereIn(shopUserIds())` reads-এ, `staff_permission:landing_pages` middleware রুটে)। `LandingPageOrderService::create()` landing page-এর owner resolve করে (`$page->user->shopOwnerId()`) Order তৈরি করে, `$page->user_id` সরাসরি trust করে না — কারণ landing page staff তৈরি করতে পারে কিন্তু downstream Order সবসময় shop owner-এর হতে হবে (courier credential resolution-এর জন্য)। **এই ফিচারে নতুন কাজ করার আগে CONTEXT.md §৩১ এবং `staff_team_role_context.md §3.3`/`§9.3` পড়ো** — নতুন কোনো landing-page-সম্পর্কিত endpoint/service লিখলে এই একই owner-keying নিয়ম মেনে চলতে হবে।
